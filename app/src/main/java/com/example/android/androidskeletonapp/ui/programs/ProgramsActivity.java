...
public class ProgramsActivity extends ListActivity implements OnProgramSelectionListener {
    ...
    private void observePrograms() {
        ...
        List<OrganisationUnit> organisationUnits = Sdk.d2().organisationUnitModule()
                .organisationUnits.byOrganisationUnitScope(OrganisationUnit.Scope.SCOPE_DATA_CAPTURE)
                .get();

        List<String> orgUnitIds = UidsHelper.getUidsList(organisationUnits);


        LiveData<PagedList<Program>> programs = Sdk.d2().programModule().programs
                .byOrganisationUnitList(orgUnitIds)
                .withStyle()
                .getPaged(20);
